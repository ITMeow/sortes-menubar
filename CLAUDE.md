...
Ice 是一款 macOS 菜单栏管理工具，支持隐藏/显示菜单栏项目并提供额外的自定义功能。这是一个原生 Swift 应用，需要 macOS 14+。

## 构建命令

- **构建**: 在 Xcode 中打开 `Ice.xcodeproj` 并构建 (Cmd+B)
...
### 核心模式
应用使用集中式的 `AppState` (@MainActor ObservableObject) 来持有所有管理器，并通过 SwiftUI 环境传递。入口点是 `IceApp.swift`，它创建 AppState 并将其传递给 AppDelegate。

### 核心管理器 (均在 AppState 中)
- **MenuBarManager**: 管理菜单栏分区（可见、隐藏、始终隐藏）和 Ice Bar 面板
...
### UI 组件
- **IceBar** (`Ice/UI/IceBar/`): 显示隐藏菜单栏项目的浮动面板
- **LayoutBar** (`Ice/UI/LayoutBar/`): 用于重新排列项目的拖放界面
- **IceUI** (`Ice/UI/IceUI/`): 以 "Ice" 为前缀的可复用 SwiftUI 组件
